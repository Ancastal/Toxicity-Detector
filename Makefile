...
docker_build:
	docker build --platform linux/amd64 -t ${GCR_MULTI_REGION}/${GCP_PROJECT}/${DOCKER_IMAGE_NAME}:prod .
...
docker_build_alternative:
	docker buildx build --load \
		...
		-t $(GCR_MULTI_REGION)/$(GCP_PROJECT)/$(DOCKER_IMAGE_NAME):prod .

docker_run:
	docker run --platform linux/amd64 -e PORT=8000 -p ${DOCKER_LOCAL_PORT}:8000 --env-file .env ${GCR_MULTI_REGION}/${GCP_PROJECT}/${DOCKER_IMAGE_NAME}:prod
...
docker_run_interactively:
	docker run -it \
		...
		-e PORT=8000 -p $(DOCKER_LOCAL_PORT):8000 \
		--env-file .env \
		$(GCR_MULTI_REGION)/$(GCP_PROJECT)/$(DOCKER_IMAGE_NAME):prod \
		bash

# Push and deploy to cloud

docker_push:
	docker push $(GCR_MULTI_REGION)/$(GCP_PROJECT)/$(DOCKER_IMAGE_NAME):prod
